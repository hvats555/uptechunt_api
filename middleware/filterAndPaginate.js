...
module.exports = async (pageNumber, queryLimit, model, query, select, populate, selectPopulateField) =>  {
    let page = 1;
    let limit = 10;
    ...
    try {
      results.results = await model.find(query).sort('-createdAt').select(select).limit(limit).skip(startIndex).populate(populate, selectPopulateField).exec();
